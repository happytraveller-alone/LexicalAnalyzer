 ...
 * @Date: 2021-04-26 17:18:24
 * @LastEditors: happytraveller-alone
 * @LastEditTime: 2021-04-26 19:44:32
 */
...
/****************************分析子程序，算法核心***********************/
void Scanner(int &syn, char resourceProject[], char token[],
    ...
    if (IsLetter(resourceProject[pProject])) {       //开头为字母
        ...
        while (IsLetter(resourceProject[pProject]) ||
               IsDigit(resourceProject[pProject])) {     //后跟字母或数字
            token[count++] = resourceProject[pProject];  //收集
            pProject++;                                  //下移
...
int main() {
    ...
    FILE *fp, *fp1;
    if ((fp = fopen("D:\\zyr_rc.txt", "r")) == NULL) {  //打开源程序
    ...
    if ((fp1 = fopen("D:\\zyr_compile.txt", "w+")) == NULL) {  //打开源程序
        cout << "can't open this file";
        exit(0);
    }
    while (syn != 0) {
