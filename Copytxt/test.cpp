 ...
 * @Date: 2021-04-29 17:49:51
 * @LastEditors: happytraveller-alone
 * @LastEditTime: 2021-04-29 18:21:13
 */
#include <conio.h>
#include <stdio.h>
#include <stdlib.h>
...
// proc的维数都是从1开始的
int proc[Max_Proc]
        [Max_Length];  //产生式的数组，里边存储了终结符或者非终结符对应的编号
...
// NOTE: ALYWAYS HAVE SOMETHING TO ADD
void initGrammer() {
    FILE *infile;
    char ch;
    char array[30];
...
void Analysis() {
    ...
    p = p->next;
    while (p != NULL) {
        if (p->type == AUTO || p->type == CONST || p->type == UNSIGNED ||
            p->type == SIGNED || p->type == STATIC || p->type == VOLATILE) {
            reserve[i++] = DESCRIBE;
            // Push(&s2,DESCRIBE);
        } else if (p->type == INT_VAL) {
            reserve[i++] = DIGIT;
            // Push(&s2,DIGIT);
        } else if (p->type == CHAR || p->type == DOUBLE || p->type == FLOAT ||
                   p->type == INT || p->type == LONG || p->type == SHORT ||
                   p->type == VOID) {
            reserve[i++] = TYPE;
            // Push(&s2,TYPE);
