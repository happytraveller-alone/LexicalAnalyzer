 ...
 * @Date: 2021-04-29 23:19:27
 * @LastEditors: happytraveller-alone
 * @LastEditTime: 2021-04-29 23:20:20
 */
#include <string.h>
...
class Grammar {
   private:
    ...
    string maxPrefix(string left) {
        ...
        while (firstP.find(" ") != firstP.size() - 1) {
            string temp = firstP.substr(
                0, firstP.find(" "));  // left的第一个产生式中的token
    ...
    void immediateLeftRecursionRemoval(string Ai) {
        ...
        for (auto it = production[Ai].begin(); it < production[Ai].end();
             it++) {
            string right = *it;
            string temp = right;
            temp += "#";
            ...
            //含有直接左递归:Ai'->aAi'|@
            if (strcmp(const_cast<char*>(Ai.c_str()),
                       strtok(const_cast<char*>(temp.c_str()), " #")) == 0) {
   ...
   public:
    ...
    //构造函数，读入所需的四元组
    Grammar(string fileName) { readGrammar(fileName); }
            ...
            for (string right : production[left]) {
                ...
                for (int i = 0; i < right.size(); i++) {
                    if (right[i] == '|' || right[i] == ' ' || right[i] == '#') {
                        //不是非终结，且不是空，则加入终结符号
                        if ((find(NT.begin(), NT.end(), temp) == NT.end()) &&
                            temp != "@") {
                            T.push_back(temp);
                        }
                        temp = "";
    ...
    void readGrammar(string fileName) {
        ...
        while (getline(input, line)) {
            ...
            //读取右部
            string right =
                line.substr(i + 2, line.size() - i);  //获取产生式右部
        ...
        for (int i = 0; i < NT.size(); i++) {
            ...
            for (auto it = production[Ai].begin(); it < production[Ai].end();
                ...
                for (int j = 0; j < i; j++) {
                    ...
                    if (strcmp(const_cast<char*>(Aj.c_str()),
                               strtok(const_cast<char*>(temp.c_str()), " #")) ==
                        0) {
                        flag = 1;
                        cout << Aj << " ";
                        cout << temp << endl;
                        for (auto jt = production[Aj].begin();
                             jt < production[Aj].end(); jt++) {
                            string s =
                                *jt + " " +
                                right.substr(
                                    right.find(" ") +
                                    1);  // substr(1)是从空格位置往后的子串
                            // cout<<"s:"<<s<<endl;
                ...
                //没有可替换的产生式
                if (flag == 0) newRight.push_back(right);
            }
            if (i != 0) production[Ai] = newRight;
    ...
    void leftFactoring() {
        ...
        for (int i = 0; i < NT.size(); i++) {
            ...
            if (a != "") {
                ...
                for (auto it = production[left].begin();
                     it < production[left].end(); it++) {
                    string right = *it;
                    string newRight;
                    ...
                    //产生式含a
                        ...
                        if (right.find(a) + a.size() != right.size()) {
                            newRight =
                                right.substr(right.find(a) + a.size() + 1);
                        }
                        // a后面是空的
                        else {
                            newRight = "@";
                    ...
                    for (string B : NT) {
                        ...
                        if (right.find(B) != string::npos) {
                            ...
                            if (right[right.find(B) + B.size()] != ' ' &&
                                ...
                                string temp = right.substr(right.find(B) +
                                                           B.size());  //' b
                            ...
                            // A->aEb
                                ...
                                string temp = right.substr(
                                    right.find(B) + B.size() + 1);  // B后的子串

                                // b后无字符
                                if (temp.find(" ") == string::npos) b = temp;
                                // b后有字符
                                else
                                    b = temp.substr(0, temp.find(" "));
                            }
    ...
    int hasIntersection(set<string> first, set<string> second) {
        for (string b : second) {
            ...
            if (first.find(b) != first.end()) return 1;
        }
        return 0;
    }
            ...
            for (string apro : production[A]) {
                ...
                for (string bpro : production[A]) {
                    ...
                    if (apro != bpro) {
                        ...
                        // FIRST有交集，不是LL(1)
                        if (hasIntersection(FIRST[a], FIRST[b])) return 0;

                        //如果FIRST[a]中有@,FIRST[b]和FOLLOW[A]相交，则不是LL(1)文法
                        if (FIRST[a].find("@") != FIRST[a].end()) {
                            if (hasIntersection(FIRST[b], FOLLOW[A])) return 0;
                        }

                        if (FIRST[b].find("@") != FIRST[b].end()) {
                            if (hasIntersection(FIRST[a], FOLLOW[A])) return 0;
                        }
                    }
                }
            }
    ...
    int parsing(string input) {
        ...
        string nextInput =
            input.substr(0, input.find(" "));  //下一个读入的token
        while (Analysis.top() != "#" && input != "#") {
            ...
            if (find(T.begin(), T.end(), top) != T.end() && nextInput == top) {
                ...
                while (ast_stack.size() > 1) {
                    ...
                    TreeNode Node = (TreeNode)
                        ast_stack[ast_stack.size() - 2][parent_idx + 1];
            ...
            //推导
                     ...
                     find(T.begin(), T.end(), nextInput) != T.end()) {
                pair<string, string> symbol;
                ...
                if (!Table[symbol].empty()) {
                    ...
                    //产生式右部
                    string pdc =
                        Table[symbol].substr(Table[symbol].find("->") + 2);
        ...
        //情况：Table[S2,"#"]=S2->@
        while (find(NT.begin(), NT.end(), Analysis.top()) != NT.end() &&
               input == "#") {
            pair<string, string> symbol;
    ...
    void printFIRST() {
        ...
        for (string non_terminal : NT) {
            ...
            for (string first : FIRST[non_terminal]) cout << first << " ";
            cout << endl;
        }
        cout << endl;
    }
    ...
    void printFOLLOW() {
        ...
        for (string non_terminal : NT) {
            cout << setw(20) << non_terminal;
            for (string follow : FOLLOW[non_terminal]) cout << follow << " ";
            cout << endl;
        }
        cout << endl;
    }
