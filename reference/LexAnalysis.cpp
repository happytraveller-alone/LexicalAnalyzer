    ...
    while (ch != EOF) {
            ...
            //处理"/**/"型注释
                ...
                while (1) {
                    while (ch != '*') {
                        ...
                        ch = fgetc(infile);
                        if (ch == EOF) {
                            createNewError(_NULL, NOTE_ERROR, NOTE_ERROR_NUM,
                                           line);
                            return;
                        }
                    }
        ...
        //处理常量字符
            ...
            //处理-开头的运算符
                ...
                if (ch >= '0' && ch <= '9') {
                    ...
                    if (flag == 1) {
                        ...
                        createNewError(word, DOUBLE_ERROR, DOUBLE_ERROR_NUM,
                                       line);
                    } else {
                        if (flag2 == 0) {
                            createNewNode(word, CONSTANT_DESC, INT_VAL, -1,
                                          line);
                        } else {
                            createNewNode(word, CONSTANT_DESC, FLOAT_VAL, -1,
                                          line);
                        }
                    }
                    fseek(infile, -1L, SEEK_CUR);  //向后回退一位
            ...
            //处理~开头的运算符
                ...
                ch = fgetc(infile);
                if (ch == '=') {
                    createNewNode("~=", OPE_DESC, COMPLETE_COMPLEMENT, -1,
                                  line);
                } else {
